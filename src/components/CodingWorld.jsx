  ...
  return (
    <section id="coding-world" className="overflow-hidden py-16 sm:py-24">
      <div className="mx-auto max-w-7xl px-5 sm:px-10">
        ...
        <Reveal variant="up" delay={0.05}><h2 className="display text-5xl font-700 leading-none text-ink sm:text-7xl">My Coding World</h2></Reveal>
      ...
      <div className="mt-12 flex w-max animate-marquee gap-5 pr-5 hover:[animation-play-state:paused]">
        {row.map((p, i) => (
          <Link key={i} to={`/project/${p.id}`}
            ...
            <div className="mb-4 grid h-32 place-items-center overflow-hidden rounded-md tech-grid">
              {p.image_url ? (
                <img src={p.image_url} alt={p.title} className="h-full w-full object-cover" loading="lazy" />
              ) : (
                <span className="display text-5xl font-900 text-line">{p.title.split(' ').map((w) => w[0]).join('').slice(0, 3)}</span>
              )}
            </div>
